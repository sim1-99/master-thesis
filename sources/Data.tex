...
The atlas features three parcellations of \numlist{9;17;91} regions of interest. Here, the 17-label parcellation is considered, because the labels can be merged to match exactly the 7-label parcellation used in the Kispi dataset (see Tab.\,\ref{tab:label_merge} in Appendix \hyperref[app:SupplementaryTables]{B}). The segmentation is based on the dHCP structural pipeline\,\cite{Makropoulos2018, dHCP_pipeline}---an example is shown in Fig.\,\ref{fig:dhcp_images}. Some ground-truth segmentations are missing, bringing the number of volumes to \num{267}.
